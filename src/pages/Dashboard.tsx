...
import { FaBoxes, FaExclamationTriangle, FaClipboardList, FaFileAlt, FaArrowUp } from "react-icons/fa"
...
const Dashboard = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      >
        {/* Total de productos */}
        <motion.div variants={itemVariants} className="bg-neutral-white rounded-lg shadow-md p-6 flex items-center">
        ...
        <motion.div variants={itemVariants} className="bg-neutral-white rounded-lg shadow-md p-6 flex items-center">
          ...
          <div>
            ...
            <p className="text-2xl font-bold text-primary-lighter">{totalCategories}</p>
          </div>
        </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Productos recientes */}
        <motion.div variants={itemVariants} className="bg-neutral-white rounded-lg shadow-md overflow-hidden">
          <div className="bg-primary-lightest px-6 py-4 border-b border-primary-lightest flex justify-between items-center">
            <h2 className="text-lg font-semibold text-primary">Productos Recientes</h2>
            <Link to="/products" className="text-primary hover:text-primary-light text-sm font-medium">
              Ver todos
            </Link>
          </div>
          <div className="p-4">
            {recentProducts.length > 0 ? (
                ...
                {recentProducts.map((product) => (
                  <div key={product.id} className="py-3 flex items-center justify-between">
                    <div className="flex items-center">
                      <div className="ml-0">
                        <p className="text-sm font-medium text-neutral-dark">{product.name}</p>
                        <p className="text-xs text-neutral-medium">{product.category}</p>
                      </div>
                    </div>
              ...
              <p className="text-neutral-medium text-center py-4">No hay productos registrados aún</p>
            )}
          </div>
        </motion.div>
        ...
        <motion.div variants={itemVariants} className="bg-neutral-white rounded-lg shadow-md overflow-hidden">
          <div className="bg-accent-light px-6 py-4 border-b border-accent-light flex justify-between items-center">
            <h2 className="text-lg font-semibold text-primary">Retiros Recientes</h2>
            <Link to="/withdrawals" className="text-primary hover:text-primary-light text-sm font-medium">
              Ver todos
            </Link>
          </div>
          <div className="p-4">
            {recentWithdrawals.length > 0 ? (
              ...
              <p className="text-neutral-medium text-center py-4">No hay retiros registrados aún</p>
            )}
          </div>
        </motion.div>
      </div>
      ...
      <motion.div variants={itemVariants} className="bg-neutral-white rounded-lg shadow-md overflow-hidden">
        ...
        <div className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <h3 className="text-md font-medium mb-3 text-primary">Principales Categorías</h3>
                ...
                {Object.entries(
                  ...
                  .map(([category, count]) => (
                    <div key={category} className="flex items-center justify-between">
                      ...
                      <div className="flex items-center">
                        <span className="text-sm font-medium mr-2 text-neutral-medium">{count}</span>
            ...
            <div>
              <h3 className="text-md font-medium mb-3 text-primary">Productos con mayor retiro</h3>
              <div className="space-y-2">
                {topWithdrawnProducts.map((product) => (
                  <div key={product.id} className="flex items-center justify-between">
                    <p className="text-sm truncate max-w-[180px] text-neutral-dark">{product.name}</p>
                    <div className="flex items-center">
